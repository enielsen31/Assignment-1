...
void list_insert(struct list* list, void* val)
{
    ...
    if(list->head == NULL){
        list->head = new_node; //break the list
        new_node->val = val;
        new_node->next = NULL;
    } else {
        // there is stuff in the list already
        new_node->next = list->head; // Point the new node to the current head
        // Ensure that memory was successfully allocated
        new_node->val = val;
        list->head = new_node;
    }
...
{
     struct node* new_node = (struct node*)malloc(sizeof(struct node));
//    if(new_node != NULL){ // Ensure that memory was successfully allocated
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    while (current != NULL) {
        if (cmp(val, current->val) == 0) {  // Found the node to remove
            if (previous == NULL) {
                // Removing the head
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    while (current != NULL) {
        if (cmp(val, current->val) == 0) {
            return index;  // Found the value, return the position
        }
        current = current->next;
        index++;
    }
...
void list_reverse(struct list* list)
{
    ...
    list->head = previous;  // Update head to the last node
    /*
     * FIXED ME:
     */
    return;
