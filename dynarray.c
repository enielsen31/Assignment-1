...
struct dynarray* dynarray_create(){
    struct dynarray* da = (struct dynarray*)malloc(sizeof(struct dynarray)); // Allocate memory for the dynamic array
    if (da == NULL) {
        return NULL; // return null when if memory allocation fails
    }

    da->data = malloc(2 * sizeof(void*)); // Initialize the array with a capacity of 2
    if (da->data == NULL) {
        free(da); //
        return NULL;
    }

    da->size = 0; // initially empty
    da->capacity = 2; // initial capacity
    return da;
    /*
     * FIXED ME:
     */
...
void dynarray_free(struct dynarray* da){
    ...
    free(da);        // Free the dynamic array
    /*
     * FIXED ME:
     */
    return;
...
void dynarray_insert(struct dynarray* da, void* val){
    if (da->size == da->capacity) {
        da->capacity *= 2;
        da->data = realloc(da->data, da->capacity * sizeof(void*)); // Resize the array
        if (da->data == NULL) {
    ...
    da->data[da->size] = val; // Insert the new value at the end
...
void dynarray_remove(struct dynarray* da, int idx){
     // Shift elements left to fill the gap created by removal
    for (int i = idx; i < da->size - 1; i++) {
        da->data[i] = da->data[i + 1];
    }
    da->size--; // Decrease the size of the array
    /*
     * FIXED ME:
     */
    return;
...
void dynarray_set(struct dynarray* da, int idx, void* val){
    ...
    da->data[idx] = val; // Set the new value
    /*
     * FIXED ME:
     */
    return;
